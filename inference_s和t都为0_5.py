...
from models.MobileNet_0_5_new import mobilenet_0_5_new
...
if __name__ == '__main__':
    ...
    '''这里用0.5的mobilenet的后半做实验，计算准确率
    同一个网络确实没有问题，但是不同网络特征图差距还是很大'''
    model_t_new = mobilenet_0_5_new()

    new_state_dict = copy.deepcopy(state_dict_s)
    for key in state_dict_s:
        if key not in model_t.state_dict():
            new_state_dict.pop(key)
    model_t_new.load_state_dict(new_state_dict)


    args.device = torch.device('cpu')
    model_s = model_s.to(args.device)
    model_t_new = model_t_new.to(args.device)
    ...
    J = 100
    threshold = 0.9  # 置信度超过这个值，则直接分类，不进行传递
