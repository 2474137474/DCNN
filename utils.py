...
def get_networks(args):
    if args.stu == 'MobileNet_0_5' and args.tea == 'MobileNet_1_0':
        ...
        net_stu = mobilenet_0_5(class_num=10)
        net_tea = mobilenet_1_0(class_num=10)
...
# def loss_kd_output(output_s,output_t, t):
#     """
...
def loss_kd_output(output_s, output_t, t):
    output_s /= t
    output_t /= t
    output_s = torch.softmax(output_s, dim=1)
    output_t = torch.softmax(output_t, dim=1)
    # 如果两者比较接近，则值越大，加了负号就越小
    loss = -torch.mean(torch.sum(output_t * output_s, dim=1))
    return loss
